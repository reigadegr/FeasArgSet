...
auto readProfile(const char *profile, std::vector<listGame> &profit, std::string &middle_big_core_in_game,
    ...
    while (std::getline(file, buf)) {
        ...
        if (buf[0] != '#' && buf[0] != '$' && buf[0] != '&') {
            ...
            iss >> name >> fps >> scaling_a >> scaling_b >> normal_frame_keep_count >> min_freq_limit_level >>
                max_freq_limit_level;
            // 错误值处理

            if (fps != 60 && fps != 90 && fps != 120) {
                int closest = std::min({std::abs(fps - 60), std::abs(fps - 90), abs(fps - 120)});
...
auto readPathProfile(const char *pathProfile, struct FeasPath *p) -> bool {
    ...
    Config const configs[] = {{"Feas_switch_path", &p->Feas_switch, "Feas开关节点"},
